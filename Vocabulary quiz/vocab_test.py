...
def interface_flush():
	...
	lLable_for_question.configure(background="")
...
def start_quiz():
	...
	for i in call[0]:
		...
		lLable_for_indicate_start.configure(text="Press \"Start\" to begin") 
		...
		if random_number == 0:
			...
			finde_index = call[0].index(i)
			lLable_for_indicate_start.configure(text = "{} out of {}".format(finde_index + 1, call[3]))

			bButton_for_answer.wait_variable(button_pressed)
			
			if eEntry_answer.get().lower() in call[2][finde_index] and len(eEntry_answer.get()) > 1:
...
def help_func():
	...
	lable = Label(help_window, text=help_text, font = "Arial 10")
	lable.pack()
...
filemenu.add_command(label="Просмотр файла", command = open_a_words_file)
filemenu.add_command(label="Справка", command=help_func)
...
lLable_for_indicate_start = Label(frame1, text="", font = "Arial 10")
lLable_for_indicate_start.pack(pady=5)

lLable_for_question = Label(frame1, font = "Arial 15")
lLable_for_question.pack(side=TOP, padx = 20)

llable_for_annouce = Label(frame1, font = "Arial 10")
llable_for_annouce.pack(pady=7)
